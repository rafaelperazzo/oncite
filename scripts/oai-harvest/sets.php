...
//    echo "<pre>" ; // . htmlspecialchars($setsxml) . "</pre>";
	...
	if(preg_match_all('|<set\s*>\s*<setSpec\s*>(.*)</setSpec>\s*<setName\s*>(.*)</setName>\s*</set>|Us', $setsxml, $matches, PREG_SET_ORDER))
	{
	  ...
	  <input type="hidden" name="action" value="storesets" />
	  <input type="submit" />
	  </form>
	  <?php
	  $showmainmenu = false;
...
if($showmainmenu)
{
  ...
  <p>&nbsp;</p>
  <ul>
    <li><a href='sets.php?action=listsets'><strong>List</strong> the sets in MyOPIA's lookup table</a></li>
    <li><a href='sets.php?action=clearout' onclick="return confirm('Are you sure you want to remove ALL the entries from the lookup table?')"><strong>Clear</strong> out MyOPIA's lookup table</a></li>
    <li><a href='sets.php?action=getsets'><strong>Build new lookups</strong> based on the list of sets used in the OAI repository</a></li>
  </ul>
  <p>&nbsp;</p>
